                                      ...
                                      size_t pixel_count, size_t stride) noexcept
{
    ...
    for (; i < pixel_count; ++i)
    {
        output[i] = rgb_planar[i] - rgb_planar[stride + i] + 32768;
        output[stride + i] = rgb_planar[stride + i];
        output[2 * stride + i] = rgb_planar[2 * stride + i] - rgb_planar[stride + i] + 32768;
    }
}
                                    ...
                                    size_t pixel_count, size_t stride) noexcept
{
    ...
    for (; i < pixel_count; ++i)
    {
        rgb_planar[i] = input[i] + input[stride + i] - 32768;
        rgb_planar[stride + i] = input[stride + i];
        rgb_planar[2 * stride + i] = input[2 * stride + i] + input[stride + i] - 32768;
    }
}
                                ...
                                size_t pixel_count, size_t stride) noexcept
{
    ...
    for (; i < pixel_count; ++i)
    {
        output[i] = rgb_planar[i] - rgb_planar[stride + i] + 128;
        output[stride + i] = rgb_planar[stride + i];
        output[2 * stride + i] = rgb_planar[2 * stride + i] - rgb_planar[stride + i] + 128;
    }
}
                                ...
                                size_t pixel_count, size_t stride) noexcept
{
    ...
    for (; i < pixel_count; ++i)
    {
        output[i] = rgb_planar[i] - rgb_planar[stride + i] + 128;
        output[stride + i] = rgb_planar[stride + i];
        output[2 * stride + i] = rgb_planar[2 * stride + i] - ((rgb_planar[i] + rgb_planar[stride + i]) >> 1) - 128;
    }
}
                                      ...
                                      size_t pixel_count, size_t stride) noexcept
{
    ...
    for (; i < pixel_count; ++i)
    {
        output[i] = rgb_planar[i] - rgb_planar[stride + i] + 32768;
        output[stride + i] = rgb_planar[stride + i];
        output[2 * stride + i] = rgb_planar[2 * stride + i] - ((rgb_planar[i] + rgb_planar[stride + i]) >> 1) - 32768;
    }
}
                                    ...
                                    size_t pixel_count, size_t stride) noexcept
{
    ...
    for (; i < pixel_count; ++i)
    {
        const uint16_t r = input[i] + input[stride + i] - 32768;
        rgb_planar[i] = r;
        rgb_planar[stride + i] = input[stride + i];
        rgb_planar[2 * stride + i] = input[2 * stride + i] + ((r + input[stride + i]) >> 1) - 32768;
    }
}
                                ...
                                size_t pixel_count, size_t stride) noexcept
{
    ...
    for (; i < pixel_count; ++i)
    {
        const uint8_t v2 = rgb_planar[2 * stride + i] - rgb_planar[stride + i] + 128;
        const uint8_t v3 = rgb_planar[i] - rgb_planar[stride + i] + 128;
        output[i] = rgb_planar[stride + i] + ((v2 + v3) >> 2) - 64;
        output[stride + i] = v2;
                              ...
                              size_t pixel_count, size_t stride) noexcept
{
    ...
    for (; i < pixel_count; ++i)
    {
        ...
        const uint8_t g = v1 - ((v3 + v2) >> 2) + 64;
        rgb_planar[stride + i] = g;
        rgb_planar[i] = v3 + g - 128;
        rgb_planar[2 * stride + i] = v2 + g - 128;
    }
}
#endif
                                 ...
                                 uint8_t* rgb, size_t pixel_count) noexcept
{
    ...
    for (; i < simd_end; i += 16)
    {
        const __m128i r_vec = _mm_loadu_si128(reinterpret_cast<const __m128i*>(r + i));
        const __m128i g_vec = _mm_loadu_si128(reinterpret_cast<const __m128i*>(g + i));
        const __m128i b_vec = _mm_loadu_si128(reinterpret_cast<const __m128i*>(b + i));
        
        // Interleave using unpack operations
        // First interleave R and G: RG RG RG RG...
        const __m128i rg_lo = _mm_unpacklo_epi8(r_vec, g_vec);  // r0g0r1g1r2g2r3g3...
        const __m128i rg_hi = _mm_unpackhi_epi8(r_vec, g_vec);
        
        // Then interleave RG with B (need 16-bit unpack)
        const __m128i b_lo = _mm_unpacklo_epi8(b_vec, _mm_setzero_si128());
        const __m128i b_hi = _mm_unpackhi_epi8(b_vec, _mm_setzero_si128());
        
        // This creates: r0g0b0 r1g1b1 ... but we need proper byte arrangement
        // Simplified approach: use shuffles or scalar for complex interleaving
        // For now, use partially vectorized approach
        
        // Store interleaved result (simplified scalar for complex pattern)
                                       ...
                                       uint16_t* rgb, size_t pixel_count) noexcept
{
    ...
    for (; i < simd_end; i += 8)
    {
        ...
        // Scalar fallback loop for the 8 pixels to keep implementation simple and correct
        // (Vector shuffle for 3-channel 16-bit is complex)
